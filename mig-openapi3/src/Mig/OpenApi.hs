...
toOperation routeInfo =
  ...
  where
    ...
    addInput = \case
      Info.QueryInput queryName querySchema -> addParam $
        mempty
          & name .~ queryName
          & required ?~ True
          & in_ .~ ParamQuery
      ...
      Info.OptionalInput queryName querySchema -> addParam $
        mempty
          & name .~ queryName
          & required ?~ False
          & in_ .~ ParamQuery
      ...
      Info.CaptureInput captureName captureSchema -> addParam $
        mempty
          & name .~ captureName
          & required ?~ True
          & in_ .~ ParamPath
      ...
      Info.HeaderInput headerName headerSchema -> addParam $
        mempty
          & name .~ headerName
          & required ?~ True
          & in_ .~ ParamHeader
