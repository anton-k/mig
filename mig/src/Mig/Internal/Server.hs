...
import Mig.Internal.Api (Api, toNormalApi, fromNormalApi)
  ...
  where
    ...
    goPath n (Api.Path path) api = case path of
      ...
      Api.CapturePath "*" : rest ->
        let (nextRest, m) = goPath (n + 1) (Api.Path rest) api
        in
      ...
      a : rest ->
        let (nextRest, m) = goPath n (Api.Path rest) api
